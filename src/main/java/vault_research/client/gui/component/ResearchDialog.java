package vault_research.client.gui.component;
...
import vault_research.client.gui.widget.ResearchWidget;
...
import vault_research.network.message.ResearchMessage;
...
public class ResearchDialog extends AbstractGui {
	...
	public void refreshWidgets() {
		if (this.researchName != null) {
			SkillStyle researchStyle = ModConfigs.RESEARCHES_GUI.getStyles().get(researchName);
	...
	public void research() {
		...
		int unspentPoints = VaultBarOverlay.unspentSkillPoints;
		...
		if (minecraft.player != null) {
			minecraft.player.playSound(ModSounds.SKILL_TREE_LEARN_SFX, 1f, 1f);
		}
	...
	private void renderHeading(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		SkillStyle abilityStyle = ModConfigs.RESEARCHES_GUI.getStyles().get(researchName);
		...
		FontHelper.drawStringWithBorder(matrixStack, researchName, abilityBounds.getWidth() + gap, 13,
				!researched ? 0xFF_FFFFFF : 0xFF_fff8c7, !researched ? 0xFF_000000 : 0xFF_3b3300);
